...
class SchedulerInterface(ABC):
    ...
    def convert_x0_to_velocity(
        ...
        """
        Convert the diffusion network's x0 prediction to velocity predidction.
        x0: the predicted clean data with shape [B, C, H, W]
        xt: the input noisy data with shape [B, C, H, W]
        timestep: the timestep with shape [B]
        ...
        v = sqrt(alpha_t) * noise - sqrt(beta_t) x0
        noise = (xt-sqrt(alpha_t)*x0) / sqrt(beta_t)
        given x0, x_t, we have
        v = (sqrt(alpha_t)/sqrt(beta_t)) * x_t - (1/sqrt(beta_t)) * x0
        see derivations https://chatgpt.com/share/679fb6c8-3a30-8008-9b0e-d1ae892dac56
        """
        # use higher precision for calculations
        original_dtype = x0.dtype
        ...
        velocity = (alpha_prod_t ** 0.5 / beta_prod_t ** 0.5) * xt - (1 / beta_prod_t ** 0.5) * x0
